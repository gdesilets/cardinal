...
| `strategy-report [--min-n N]` | Wins/losses/breakevens/win_rate/sample_flag per strategy, ranked highest win_rate first — this is the "which strategy is highest probability" report. Defaults to `--min-n 15` (excludes small-sample strategies from the default view; pass `--min-n 0` to see everything, including `sweep-displacement-fvg-retrace` at n=6). Always relay `decided_n` and `sample_flag` alongside `win_rate`, never the bare rate. |
