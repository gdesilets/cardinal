...
Usage:
  ...
  python query_corpus.py model 1                       # model card by model_no
  python query_corpus.py models                        # list all model cards
...
sibling discord_trading_research/discord_trading_research_3month.sqlite.
"""
import argparse
import json
import sqlite3
...
DB_RELATIVE = Path("discord_trading_research") / DB_FILENAME
...
def find_db() -> Path:
    ...
    raise FileNotFoundError(
        ...
        f"{BUNDLED_DB}, or in a discord_trading_research/ folder above the "
...
def cmd_model(con, args):
    row = con.execute("SELECT * FROM v_model_cards WHERE model_no = ?", (args.model_no,)).fetchone()
    if not row:
        print(f"No model with model_no={args.model_no}", file=sys.stderr)
        return
    d = dict(row)
    model_id = con.execute("SELECT model_id FROM trading_models WHERE model_no = ?", (args.model_no,)).fetchone()[0]
    ev = con.execute(
        "SELECT message_id, evidence_role, excerpt FROM model_evidence WHERE model_id = ?", (model_id,)
    ).fetchall()
    d["evidence"] = [dict(e) for e in ev]
...
def cmd_models(con, args):
    rows = con.execute(
        "SELECT model_no, name, evidence_status, win_count, loss_count, breakeven_count, unknown_count, instrument_scope, timeframe_scope, session_scope FROM v_model_cards ORDER BY model_no"
    ).fetchall()
    dump(rows)
...
def main():
    ...
    p = sub.add_parser("model", help="Full model card + evidence for one model_no (1-4)")
    p.add_argument("model_no", type=int)
    p.set_defaults(func=cmd_model)

    p = sub.add_parser("models", help="List all model cards (summary)")
    ...
    p = sub.add_parser("trades", help="Filter curated trade episodes")
